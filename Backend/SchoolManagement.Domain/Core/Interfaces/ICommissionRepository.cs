...
{
    /// <summary>Returns all commissions for a given earner.</summary>
    Task<List<Commission>> GetByEarnerAsync(Guid earnerId, EarnerType earnerType);

    /// <summary>Returns all commissions for a given period month.</summary>
    Task<List<Commission>> GetByPeriodAsync(DateOnly periodMonth);

    /// <summary>
    /// Returns the agent commission already recorded for the given agent + period,
    /// so the monthly job can skip agents already calculated.
    /// </summary>
    ...
    /// <summary>
    /// Returns all Approved commissions for a given period month.
    /// Used by the salary lockout job to flip them to Paid.
    /// </summary>
    Task<List<Commission>> GetApprovedByPeriodAsync(DateOnly periodMonth);

    /// <summary>
    /// Returns the OPC commission linked to a specific enrollment, if any.
