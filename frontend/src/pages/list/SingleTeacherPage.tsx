...
import { getMockTeacherById } from "@/lib/mockSchool";
...
const SingleTeacherPage = () => {
  ...
  const teacher = id ? getMockTeacherById(id) : null;

  if (!teacher) {
    return (
      <div className="p-4 m-4 bg-white rounded-md">
        <p className="text-gray-600">Teacher not found.</p>
        <Link to="/list/teachers" className="text-blue-600 text-sm mt-2 inline-block">
          Back to teachers
        </Link>
      </div>
    );
              ...
              <div className="flex items-center gap-4">
                ...
                {role === "admin" && (
                  <FormActionButtonLink
                    to={`/list/teachers/${teacher.id}/edit`}
                    variant="update"
                  />
                )}
              </div>
        ...
        <div className="bg-white p-4 rounded-md">
          ...
          <div className="mt-4 flex gap-4 flex-wrap text-xs text-gray-500">
            ...
            <Link
              ...
              to={`/list/students?teacherId=${teacher.id}`}
            >
              Teacher&apos;s Students
            </Link>
            <Link
              className="p-3 rounded-md bg-lamaYellowLight"
              to={`/list/lessons?teacherId=${teacher.id}`}
            >
              Teacher&apos;s Lessons
            </Link>
            <Link
              className="p-3 rounded-md bg-pink-50"
              to={`/list/exams?teacherId=${teacher.id}`}
            >
              Teacher&apos;s Exams
            </Link>
            <Link
              className="p-3 rounded-md bg-lamaSkyLight"
              to={`/list/assignments?teacherId=${teacher.id}`}
            >
